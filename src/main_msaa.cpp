...
Camera camera_msaa(glm::vec3(0.0f,0.0f,3.0f));
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
        shader.use();
        glm::mat4 projection = glm::perspective(glm::radians(camera_msaa.Zoom), (float)SCR_WIDTH/(float)SCR_HEIGHT, 0.1f, 1000.0f);
        shader.setMat4("projection", projection);
        shader.setMat4("view",camera_msaa.GetViewMatrix());
        glm::mat4 model(1.0f);
